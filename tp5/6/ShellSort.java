...
class Lista {
	...
	*/
	 ...
	 * @param int shell shell do pseudo array.
	 * @param int h passo do shelsort
	 */
	public void shellsort(int shell, int h) {
		for (int i = (h + shell); i < n; i += h) {
			Muni tmp = array[i];
			int j = i - h;
			while ( ((j >= 0) && ( array[j].getUf().compareTo(tmp.getUf() ) > 0))
			        || ((j >= 0) && (array[j].getUf().compareTo(tmp.getUf()) == 0) && (array[j].getId() > tmp.getId()))) {
				countComp++;
				array[j + h] = array[j];
				j -= h;
			}
...
class Log {
	...
	public static void createFile(int countComp, long fim, long comeco, int countMov) {
		...
		try {
			writer = new PrintWriter("504654_shellsort.txt", "UTF-8");
