	...
	public void bubbleSort() {
		...
		while (troca) {
			...
			for (int i = 0; i < n - 1; i++) {
				...
				if (array[i].getEscolaridade().compareTo(array[i + 1].getEscolaridade()) > 0) {
					swap(i, i + 1);
					troca = true;
				} 
				else if (array[i].getEscolaridade().compareTo(array[i + 1].getEscolaridade()) == 0) {
...
class Log {
	...
	public static void createFile(int countComp, long fim, long comeco, int countMov) {
		...
		try {
			writer = new PrintWriter("504654_bolha.txt", "UTF-8");
