...
class Lista {
	...
	*/
	...
	/**
	* metodo de chamada auxiliar ao selecao recursiva
	*/
	...
	/**
	 * metodo de selecao recursiva
	 * @param c contador start
	 */
	public void selecaoRecursiva(int c) {
		if (c == n - 1) {
			return;
		}
		int menor = c;
		for (int j = c + 1; j < n; j++) {
			if (array[menor].getAtualizacaoPlano() > array[j].getAtualizacaoPlano()) {
				menor = j;
				countComp++;
			} else if (array[menor].getAtualizacaoPlano() == array[j].getAtualizacaoPlano()) {
				countComp++;
...
class Log {
	...
	public static void createFile(int countComp, long fim, long comeco, int countMov) {
		...
		try {
			writer = new PrintWriter("504654_selecao.txt", "UTF-8");
