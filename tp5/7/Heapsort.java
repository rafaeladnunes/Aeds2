...
class Lista {
	...
	*/
	 ...
	 * @param tamHeap tamanho do heap recebido
	 */
	public void makeHeap(int tamHeap) {
		countComp++;
		for (int i = tamHeap; i > 1 && array[i - 1].getCodigoUf() > array[i / 2 - 1].getCodigoUf() ||
		        i > 1 && array[i - 1].getCodigoUf() == array[i / 2 - 1].getCodigoUf() &&
		        array[i - 1].getId() > array[i / 2 - 1].getId()  ; i /= 2) {
			countComp++;
			swap(i - 1, i / 2 - 1);
			countMov += 2;
		}
	...
	/**
	 * Metodo de remorcao do heap
	 * @param  tamHeap tamanho do heap recebido
	 * @return         retorno do objeto
	 */
	public Muni removeHeap(int tamHeap) {
		Muni removido = array[0];
		int aux;
		...
		for (int i = 1; i <= (tamHeap / 2); i = aux) {
			boolean condicao = (2 * i == tamHeap) || array[(2 * i) - 1].getCodigoUf() > array[(2 * i + 1) - 1].getCodigoUf() ||
			                   array[(2 * i) - 1].getCodigoUf() == array[(2 * i + 1) - 1].getCodigoUf() &&
			                   array[(2 * i) - 1].getId() > array[(2 * i + 1) - 1].getId();
			countComp++;
			aux = (2 * i) + ((condicao) ? 0 : 1);

			countComp++;
			if (array[i - 1].getCodigoUf() < array[aux - 1].getCodigoUf() || array[i - 1].getCodigoUf() == array[aux - 1].getCodigoUf() &&
			        array[i - 1].getId() < array[aux - 1].getId()) {
				swap(i - 1, aux - 1);
				countMov += 2;
			} else {
				aux = tamHeap;
	...
	public  void heapsort() {
		...
		for (int tamHeap = n; tamHeap > 0; tamHeap--) {
			array[tamHeap - 1] = removeHeap(tamHeap);
			countMov++;
...
class Log {
	...
	public static void createFile(int countComp, long fim, long comeco, int countMov) {
		...
		try {
			writer = new PrintWriter("504654_heapsort.txt", "UTF-8");
