...
class Lista {
	...
	*/
	...
	/**
	 * Metodo busca o maior objeto do array
	 * @return retorno do trem
	 */
	public Muni getMaior() {
		Muni maior = array[0];

		for (int i = 0; i < n; i++) {
			if (maior.getNumeroFuncionarios() < array[i].getNumeroFuncionarios()) {
				countComp++;
				maior = array[i];
			}
		}
	...
	public void countingsort() {
		...
		int[] count = new int[getMaior().getNumeroFuncionarios() + 1];
		Muni[] ordenado = new Muni[n];
		...
		for (int i = 0; i < n; array[i] = ordenado[i], i++, countMov++);
	...
	public  void mergeSort(Muni[] array) {
		if (array.length > 1) {
			// split array into two halves
			Muni[] left = leftHalf(array);
			Muni[] right = rightHalf(array);
	...
	public  Muni[] leftHalf(Muni[] array) {
		int size1 = array.length / 2;
		Muni[] left = new Muni[size1];
	...
	public  Muni[] rightHalf(Muni[] array) {
		int size1 = array.length / 2;
		int size2 = array.length - size1;
		Muni[] right = new Muni[size2];
	...
	public  void merge(Muni[] result, Muni[] left, Muni[] right) {
		int i1 = 0;   // index into left array
		int i2 = 0;   // index into right array

		for (int i = 0; i < result.length; i++) {
			if (i2 >= right.length || (i1 < left.length && left[i1].getNumeroComissionados() <= right[i2].getNumeroComissionados())) {
				result[i] = left[i1];    // take from left
				i1++;
			}
			else if(i2 >= right.length || (i1 < left.length && left[i1].getNumeroComissionados() == right[i2].getNumeroComissionados() && left[i1].getId() < right[i2].getId())){
				result[i] = left[i1];    // take from left
				i1++;
			} 
			else {
...
class Log {
	...
	public static void createFile(int countComp, long fim, long comeco, int countMov) {
		...
		try {
			writer = new PrintWriter("504654_mergesort.txt", "UTF-8");
	...
	*/
	...
	public static void main(String[] args) throws Exception {
		...
		while (Integer.parseInt(str) != 0) {
			arrayInsercao[tamanhoLista] = Integer.parseInt(str);
			tamanhoLista++;
			str = MyIO.readLine();
		}
