...
class Muni {
	...
	public int getDoidao() {
		...
		return (atualizacaoPlano*1000)+getId();
	}
...
class Lista {
	...
	*/
	...
	public void radixSort(){
        ...
        int i; 
        int doid = array[0].getDoidao(); 
        int aux = 1;
       
        Muni[] muDoid = new Muni[n];
        
        for (i = 1; i < n; i++)
        
            if (array[i].getDoidao() > doid)
                doid = array[i].getDoidao();
        ...
        while (doid / aux > 0)
        {
            ...
            for (i = 0; i < n; i++)
                arrayAux[(array[i].getDoidao() / aux) % 10]++;
            for (i = 1; i < 10; i++)
                arrayAux[i] += arrayAux[i - 1];
            for (i = n - 1; i >= 0; i--)
                muDoid[--arrayAux[(array[i].getDoidao() / aux) % 10]] = array[i];
            for (i = 0; i < n; i++)
                array[i] = muDoid[i];
            aux *= 10;        
        }
...
class Log {
	...
	public static void createFile(int countComp, long fim, long comeco, int countMov) {
		...
		try {
			writer = new PrintWriter("504654_radix.txt", "UTF-8");
