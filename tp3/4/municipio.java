...
class FilaCircular {
	...
	/**
	 * construtor filaCircular
	 * @param  tamanho tmanho maximo da fila
	 * @return         retorno
	 */

	public FilaCircular(int tamanho) {
		dados = new Muni[tamanho];
		tamanhoMax = tamanho;
	}

	/**
	 * metodo de inserçao
	 * @param  x         recebe a pessoa a inserir
	...
	/**
	 * metodo de remorçao do pessoa
	 * @return retorna a classe com elemento removido
	 * @throws Exception     retorno de erro
	 */
	...
	public double mediaSomatorio() {
		...
		for (int i = 0; i < tamanhoMax; i++) {
			if (dados[i] != null) {
				resultado += dados[i].getPopulacao();
			}
		}
	...
	/**
	 * media arredondada
	 * @param  resultado input
	 * @return           output
	 */

	public int arredondamento(double resultado) {
		int valor = (int)resultado;
		double decimal = resultado - valor;
		if (decimal >= 0.5) {
			valor = valor + 1;
		} else if (decimal < 0.5) {
			valor = valor;
