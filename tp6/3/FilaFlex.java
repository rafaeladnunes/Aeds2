...
class Fila {
	...
	/**
	 * Metodo efetua a media do somatorio 
	 * @return retorno
	 */
	public double mediaSomatorio() {
		...
		for (Celula i = primeiro.prox; i != null; i = i.prox) {
			soma += i.elemento.getPopulacao();
			count++;
		}

		result = Math.round(soma / count);

		return result;
	}

	/**
	 * metodo converte a media em double para int
	 * @return retorno
	 */
	public int media(){
		int resp = 0;

		resp = (int) mediaSomatorio();
